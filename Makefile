...
COMMON_LIBS += ws2_32 winmm dxguid uuid ole32
GL_LIBS     += opengl32
ifeq ($(DEBUG),Y)
CL_LFLAGS += -mconsole
else
...
quiet_cmd_iconheader = '  GENERATE $@'
      cmd_iconheader = convert $< -resize $(1)x$(2) -define h:format=rgba -depth 8 -size $(1)x$(2) $@ 2>/dev/null || ( echo "WARNING: 'convert' not found, skipping icon generation" >&2; printf "\#define DISABLE_ICON\n" >> $@ )
